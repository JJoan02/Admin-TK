...
export class AnalyticsService {
  ...
  async getAnalyticsData() {
    try {
      ...
      const messagesProcessedResult = await db.get(
        `SELECT COUNT(*) as total FROM analytics WHERE event_type = 'message_processed'`
      );
      const messagesProcessed = messagesProcessedResult?.total || 0; // Acceso seguro

      // Contar la ejecución de comandos por nombre
      const commandExecutions = await db.all(
        `SELECT JSON_EXTRACT(event_data, '$.command') as commandName, COUNT(*) as total
         FROM analytics
      ...
      const activeUsersResult = await db.get(
        `SELECT COUNT(DISTINCT JSON_EXTRACT(event_data, '$.userJid')) as total
         FROM analytics
