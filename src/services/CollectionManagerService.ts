...
export class CollectionManagerService {
  ...
  private scraper: PinterestScraperService;
  ...
  async scrapePinterestImage(keywords: string[]): Promise<any | null> {
  ...
  async downloadAndSaveImage(imageUrl: string, type: string, userJid: string): Promise<any> {
  ...
  async getUserCoins(userJid: string): Promise<number> {
    const db = DatabaseService.getInstance();
    const result = await db.get('SELECT coins FROM user_coins WHERE jid = ?', [userJid]);
    return result?.coins || 0;
  }

  async addUserCoins(userJid: string, amount: number): Promise<void> {
    const db = DatabaseService.getInstance();
  ...
  async removeUserCoins(userJid: string, amount: number): Promise<void> {
    const db = DatabaseService.getInstance();
    await db.run('UPDATE user_coins SET coins = coins - ? WHERE jid = ?', [amount, userJid]);
  }
}