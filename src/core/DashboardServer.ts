...
export class DashboardServer {
  ...
  private setupMiddleware(): void {
    ...
    this.app.use((req: Request, res: Response, next: NextFunction) => {
      ...
      if (domainConfig.allowedOrigins.includes(origin as string)) {
  ...
  private setupRoutes(): void {
    ...
    this.app.post('/api/bots', async (req: Request, res: Response) => {
      try {
        ...
        const botConfig: Omit<SubBotConfig, 'id' | 'createdAt'> = {
          name,
          phoneNumber,
          userId,
    ...
    this.app.post('/api/bots/:botId/start', async (req: Request, res: Response) => {
      try {
    ...
    this.app.post('/api/bots/:botId/stop', async (req: Request, res: Response) => {
      try {
        const botId = req.params.botId;
        if (!botId) {
          return res.status(400).json({
    ...
    this.app.get('/api/bots/:botId/logs', async (req: Request, res: Response) => {
      try {
        const botId = req.params.botId;
        ...
        const logs: any[] = []; // await this.getBotLogs(botId, parseInt(limit as string));
    ...
    this.app.get('/api/bots/:botId/qr', async (req: Request, res: Response) => {
      try {
        const botId = req.params.botId;
    ...
    this.app.get('/link/:botId', (req: Request, res: Response) => {
      const botId = req.params.botId;
      if (!botId) {
   ...
   */
  private async sendStatsToClient(socket: any): Promise<void> {
    try {
      const stats = await this.getDashboardStats();
   ...
   */
  private async generateBotQRCode(botId: string): Promise<string> {
...
<body>
   ...
   */
  private generateLinkPageHTML(botId: string): string {
    return `
<!DOCTYPE html>
<html lang="es">
<head>
