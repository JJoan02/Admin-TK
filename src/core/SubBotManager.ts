...
export class SubBotManager extends EventEmitter {
  private bots: Map<string, SubBotConfig> = new Map();
  ...
  async createBot(config: Omit<SubBotConfig, 'id' | 'createdAt'>): Promise<string> {
    const botId = `bot_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    const botConfig: SubBotConfig = {
      ...config,
      id: botId,
      createdAt: new Date(),
      status: 'offline'
  ...
  async stopAllBots(): Promise<void> {
    const stopPromises = Array.from(this.workers.keys()).map(botId => 
      this.stopBot(botId).catch(error => 
        moduleLogger.error({ botId, error }, 'Error stopping bot during shutdown')
      )
    );
  ...
  private handleWorkerMessage(botId: string, message: any): void {
    const bot = this.bots.get(botId);
    if (!bot) return;
  ...
  private handleWorkerError(botId: string, error: Error): void {
    const bot = this.bots.get(botId);
    if (bot) {
      bot.status = 'error';
      this.emit('botStatusChanged', botId, 'error');
  ...
  private handleWorkerExit(botId: string, code: number | null, signal: string | null): void {
    const bot = this.bots.get(botId);
    if (bot) {
  ...
  private async saveBotConfig(config: SubBotConfig): Promise<void> {
    // TODO: Implementar guardado en base de datos
    moduleLogger.debug({ botId: config.id }, 'Saving bot config to database');
  }

  private async deleteBotConfig(botId: string): Promise<void> {
