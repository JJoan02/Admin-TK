...
import { CommandExecutionError } from '../core/ErrorHandler.js';
import { ProcessTextCommand } from '../commands/ProcessTextCommand.js';
import Metrics from '../utils/Metrics.js';

export default export class MessageHandler {
  #config;
  #logger;
  #errorHandler;
  #commandBus;
  ...
  _shouldIgnore({ user, isGroup, group, chat }) {
    ...
    if (isGroup && !group?.isBotEnabled) return true;
    if (!isGroup && chat?.isBotMuted) return true;
    return false;
  }

  _parseCommand(text = '') {
    if (!text.startsWith(this.#config.prefix)) return { isCommand: false };
  ...
  async _handleAutonomousAI(context) {
    ...
    if (isGroup && !(await this.#aiInterventionService.shouldIntervene(context))) return;
