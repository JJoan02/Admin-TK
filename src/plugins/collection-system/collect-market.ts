...
export class CollectMarketPlugin extends BasePlugin {
  ...
  async execute(context: any): Promise<void> {
    ...
    try {
      ...
      if (args[0] === 'sell') {
        const itemId = args[1];
        if (!itemId) return await reply('❓ Especifica el ID del item a vender');
        ...
        await db.run(`INSERT INTO market_listings 
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [user.jid, item.item_id, item.item_name, item.item_type, item.rarity, item.image_url, salePrice, new Date().toISOString()]);
        ...
        if (!listingId) return await reply('❓ Especifica el ID de la venta');

        const listing = await db.get('SELECT * FROM market_listings WHERE id = ?', [listingId]);
        if (!listing) return await reply('❌ Venta no encontrada');
        ...
        await db.run(`INSERT INTO user_collections 
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [user.jid, listing.item_id, listing.item_type, listing.item_name, 'Comprado en el mercado', 
