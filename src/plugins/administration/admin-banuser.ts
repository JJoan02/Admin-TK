...
export class adminbanuserPlugin extends BasePlugin {
  ...
  async execute(context: any): Promise<void> {
    ...
    try {
      if (!text) return await reply('⚠️ *Ingresa el @tag de algún usuario.*')
      let who
      if (m.isGroup) who = m.mentionedJid[0]
      else who = m.chat
      if (!who) return await reply('⚠️ *Ingresa el @tag de algún usuario.*')
      let users = await this.database.getData().users
      users[who].banned = true
      await reply(m.chat, `⚠️ *El usuario @${who.split('@')[0]
    } catch (error) {
