  ...
  async execute(context: any): Promise<void> {
    ...
    try {
      ...
      if (args[0] === 'give') {
        ...
        if (!targetUser) return await reply('❓ Menciona al usuario');
        if (targetUser === user.jid) return await reply('❌ No puedes darte reputación a ti mismo');
        ...
        if (lastGiven) return await reply('⏰ Solo puedes dar reputación una vez cada 24 horas al mismo usuario');
