...
export class searchsonuPlugin extends BasePlugin {
  ...
  async execute(context: any): Promise<void> {
    ...
    try {
      try {
          if (!text.includes('|')) return await reply(`Ejemplo de uso:\n.sonu título | letra | estado de ánimo | género | voz`)
          let [titulo, letra, estado, genero, voz] = text.split('|').map(v => v.trim())
      
          if (!titulo) return await reply('⚠️ El título de la canción no puede estar vacío.')
          if (!letra) return await reply('⚠️ Falta la letra de la canción.')
          if (letra.length> 1500) return await reply('⚠️ La letra no puede superar los 1500 caracteres.')
