...
export class searchcscstalkPlugin extends BasePlugin {
  ...
  async execute(context: any): Promise<void> {
    ...
    try {
      if (!text) return await reply('Ingresa un nombre de usuario, ejemplo: cscstalk ZenzzXD')
        await await reply('⏳ Obteniendo datos...')
        try {
          let username = text.trim()
          let result = await buscarPerfil(username)
          if (!result ||!result.perfil) return await reply('❌ No se pudo obtener la información 😂')
