...
let handler = async (m, { conn, command, usedPrefix, args, text, groupMetadata, isOwner, isROwner }) => {
    ...
    switch (true) {
        case isCommand1:
            let inviteCode;
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el enlace de grupo/comunidad o canal.`, m, rcanal);
            ...
            if (info) {
                ...
                let newsletterInfo;
                if (!channelUrl)
                    return await conn.reply(m.chat, "🚩 Verifique que sea un enlace de canal de WhatsApp.", m, rcanal);
                if (channelUrl) {
                    try {
                        newsletterInfo = await conn.newsletterMetadata("invite", channelUrl).catch(e => { return null; });
                        if (!newsletterInfo)
                            return await conn.reply(m.chat, "🚩 No se encontró información del canal. Verifique que el enlace sea correcto.", m, rcanal);
        ...
        case isCommand2:
            if (!isOwner || !isROwner)
                return await conn.reply(m.chat, `🚩 No tienes permiso para usar este comando.`, m, fake);
            ch;
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp que quiere que el bot siga.\n\nPuede obtener el ID usando el comando:\n${usedPrefix}superinspect enlace${txtBotAdminCh}`, m, rcanal);
        ...
        case isCommand3:
            ...
            ch;
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp que quiere que el bot deje de seguir.\n\nPuede obtener el ID usando el comando:\n${usedPrefix}superinspect enlace${txtBotAdminCh}`, m, rcanal);
            if (text.includes("@newsletter")) {
                ch = text;
            }
            else {
        ...
        case isCommand4:
            if (!isOwner || !isROwner)
                return await conn.reply(m.chat, `🚩 No tienes permiso para usar este comando.`, m, rcanal);
            ch;
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp que quiere que el bot silencie las actualizaciones.\n\nPuede obtener el ID usando el comando:\n*${usedPrefix}superinspect enlace${txtBotAdminCh}`, m, rcanal);
            if (text.includes("@newsletter")) {
                ch = text;
            }
            else {
        ...
        case isCommand5:
            ...
            ch;
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp que quiere que el bot active las actualizaciones.\n\nPuede obtener el ID usando el comando:\n*${usedPrefix}superinspect* enlace${txtBotAdminCh}`, m, rcanal);
            if (text.includes("@newsletter")) {
                ch = text;
            }
            else {
        ...
        case isCommand6:
            if (!isOwner || !isROwner)
                return await conn.reply(m.chat, `🚩 No tienes permiso para usar este comando.`, m, rcanal);
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp respondiendo a una imagen jpg/jpeg/png o agregue un enlace de imagen\n
...
*${usedPrefix + command}* 12345@newsletter https://example.com/image.jpg\n\nPuede obtener el ID usando el comando:\n*${usedPrefix}superinspect* enlace${txtBotAdminCh}`, m, rcanal);
            ...
            if (m.quoted) {
                ...
                if (!match2 && !text.match(imageUrlRegex))
                    return await conn.reply(m.chat, `*Agregué el enlace jpg/png después del ID del canal.*`, m);
                try {
            ...
            if (text.includes("@newsletter")) {
                if (!match1)
                    return await conn.reply(m.chat, `🚩 No se encontró el ID del canal.`, m, rcanal);
                ch = match1;
            }
            else {
                ch = await conn.newsletterMetadata("invite", channelUrl).then(data => data.id).catch(e => null);
        ...
        case isCommand7:
            ...
            ch;
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp que quiere que el bot elimine la imagen del canal.\n\nPuede obtener el ID usando el comando:\n${usedPrefix}superinspect enlace${txtBotAdminCh}`, m, rcanal);
            if (text.includes("@newsletter")) {
                ch = text;
            }
            else {
        ...
        case isCommand8:
            ...
            ch;
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp para que el bot reciba notificaciones en tiempo real.\n\nPuede obtener el ID usando el comando:\n*${usedPrefix}superinspect enlace${txtBotAdminCh}`, m, rcanal);
            if (text.includes("@newsletter")) {
                ch = text;
            }
            else {
...
*${usedPrefix + command}* 12345@newsletter 1`, m, rcanal);
        ...
        case isCommand10:
            if (!isOwner || !isROwner)
                return await conn.reply(m.chat, `🚩 No tienes permiso para usar este comando.`, m, rcanal);
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp para que el bot modifique el nombre del canal.*\n\n
*Parámetros de uso:
...
*${usedPrefix + command}* 12345@newsletter NombreDelcanal\n\nPuede obtener el ID usando el comando:\n*${usedPrefix}superinspect* enlace${txtBotAdminCh}`, m, rcanal);
            ...
            if (name.length > 99)
                return await conn.reply(m.chat, `🚩 El nombre del canal no puede tener más de 99 caracteres.`, m, rcanal);
            if (text.includes("@newsletter")) {
                ch = id.trim();
            }
            else {
        ...
        case isCommand11:
            if (!isOwner || !isROwner)
                return await conn.reply(m.chat, `🚩 No tienes permiso para usar este comando.`, m, rcanal);
            if (!text)
                return await conn.reply(m.chat, `🚩 Ingrese el ID o enlace de un canal de WhatsApp para que el bot modifique la descripción del canal.\n\n
Parámetros de uso:
...
//# sourceMappingURL=herramientas-inspectador.js.map