...
let handler = async (m, { conn, text, usedPrefix, command, isOwner }) => {
    ...
    if (!localContent[m.chat])
        localContent[m.chat] = { savedContent: {} };
    ...
    if (q) {
        ...
        if (isMedia) {
            ...
            if (q.mimetype.startsWith('video') && buffer.length > 30 * 1024 * 1024)
                throw '⚠️ *El video excede el límite de 30 MB. No se puede guardar.*';
            ...
            if (!q.vcard)
                throw '⚠️ *El mensaje de contacto no tiene una vCard válida.*';
        ...
        if (!hasSeparator) {
            if (isCommandFormat)
                throw `${lenguajeGB['smsAvisoMG']()}𝙍𝙀𝙎𝙋𝙊𝙉𝘿𝙀 𝘼𝙇 𝙎𝙏𝙄𝘾𝙆𝙀𝙍 𝙊 𝙄𝙈𝘼𝙂𝙀𝙉 𝘼𝙇 𝘾𝙐𝘼𝙇 𝙌𝙐𝙄𝙀𝙍𝙀 𝘼𝙂𝙍𝙀𝙂𝘼 𝙐𝙉 𝘾𝙊𝙈𝘼𝙉𝘿𝙊 𝙊 𝙏𝙀𝙓𝙏𝙊`;
            throw `${lenguajeGB['smsAvisoMG']()}  𝙐𝙎𝘼 𝘿𝙀 𝙀𝙎𝙏𝘼 𝙈𝘼𝙉𝙀𝙍𝘼:*\n*ღ ${usedPrefix + command}* <palabra clave> (responde a contenido)\n"ღ ${usedPrefix + command} <texto> - <palabra clave>*\n*ღ ${usedPrefix + command} <comando>* (responde a sticker/imagen, ej: .help)`;
        }
        content = { type: 'text',
            value: splitText[0],
...
//# sourceMappingURL=herramientas-save.js.map