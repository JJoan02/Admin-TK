...
async function upscale(imageUrl, resolution = '1080p', enhance = true) {
    ...
    if (data?.status !== 'success')
        throw new Error('Upscale gagal: ' + JSON.stringify(data));
...
const handler = async (m, { conn, usedPrefix, command }) => {
    ...
    try {
        ...
        await conn.sendFile(m.chat, result.url, result.filename || 'upscaled.jpg', `✅ Imagen mejorada con éxito:
📤 Resolución: *${result.resolution_from}* → *${result.resolution_to}*
📦 Tamaño: *${result.size_before}* → *${result.size_after}*
✨ Mejora aplicada: *${result.enhanced ? 'Sí' : 'No'}*`, m);
    }
    catch (err) {
...
//# sourceMappingURL=herramientas-upscale.js.map