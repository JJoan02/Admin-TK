...
let handler = async (m, { conn, usedPrefix, command, args, text }) => {
    ...
    if (!text)
        return conn.reply(m.chat, `🚩 Ingresa el peso nuevo de la imágen/video.`, m);
    await m.react('🕓');
    try {
        if (isNaN(text))
            return conn.reply(m.chat, 'Sólo números.', m).then(_ => m.react('✖️'));
        if (!/image\/(jpe?g|png)|video|document/.test(mime))
            return conn.reply(m.chat, `Formato no soportado.`, m);
...
//# sourceMappingURL=tools-tama%C3%B1o.js.map