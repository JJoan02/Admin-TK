...
No esperes más y lleva tu bot al siguiente nivel con nuestro servicio de alojamiento. ¡Es fácil, rápido y económico! 💪🚀`;
    ...
    if (/^bot$/i.test(m.text)) {
        ...
        if (m.text.includes('jadibot') || m.text.includes('bots') || m.text.includes('serbot') || m.text.includes('instalarbot') || m.text.includes('infobot'))
            return;
...
🐈 𝙂𝙖𝙩𝙖 𝘿𝙞𝙤𝙨 🐈`, m);
        ...
        try {
            ...
            if (!hasPrefixWithKeyword && !hasKeywordWithoutPrefix)
                return;
            let query = m.text;
...
//# sourceMappingURL=_chatbot.js.map