...
handler.before = async function (m) {
    ...
    if (db.data.users[m.sender].suit < 0)
        db.data.users[m.sender].suit = 0;
    ...
    if (room) {
        ...
        if (m.sender == room.p2 && /^(acc(ept)?|Aceptar|acerta|aceptar|gas|aceptare?|nao|Rechazar|rechazar|ga(k.)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
            ...
            if (!room.pilih)
                this.sendMessage(room.p, { text: comienzop }, { quoted: fkontak });
            if (!room.pilih2)
                this.sendMessage(room.p2, { text: comienzop2 }, { quoted: fkontak });
            room.waktu_milih = setTimeout(() => {
                ...
                if (!room.pilih && !room.pilih2)
                    this.sendMessage(m.chat, { text: iniciativa }, { quoted: fkontak });
        ...
        if (jwb && reg.test(m.text) && !room.pilih && !m.isGroup) {
            ...
            if (!room.pilih2)
                this.reply(room.p2, `${lenguajeGB['smsAvisoIIG']()}𝙀𝙇 𝙊𝙋𝙊𝙉𝙀𝙉𝙏𝙀 𝘼𝙃 𝙀𝙇𝙀𝙂𝙄𝘿𝙊, 𝙀𝙎 𝙏𝙐 𝙏𝙐𝙍𝙉𝙊 𝘿𝙀 𝙀𝙇𝙀𝙂𝙄𝙍`, fkontak, 0);
        }
        if (jwb2 && reg.test(m.text) && !room.pilih2 && !m.isGroup) {
            ...
            if (!room.pilih)
                this.reply(room.p, `${lenguajeGB['smsAvisoIIG']()}𝙀𝙇 𝙊𝙋𝙊𝙉𝙀𝙉𝙏𝙀 𝘼𝙃 𝙀𝙇𝙀𝙂𝙄𝘿𝙊, 𝙀𝙎 𝙏𝙐 𝙏𝙐𝙍𝙉𝙊 𝘿𝙀 𝙀𝙇𝙀𝙂𝙄𝙍`, fkontak, 0);
        }
...
//# sourceMappingURL=fun-suitpvp.js.map