...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
    ...
    if (args.length < 1)
        return conn.reply(m.chat, '💣 Ingresa la cantidad de ' + `🍫 *Chocolates*` + ' que deseas aportar contra' + ` *Megumin-Bot-MD*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m);
    if (user.chocolates >= count * 1) {
        ...
        conn.reply(m.chat, `No tienes *${formatNumber(count)} 🍫 Chocolates* para apostar!`.trim(), m);
};
...
//# sourceMappingURL=fun-casino.js.map