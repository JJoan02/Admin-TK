...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (command === 'reclamar') {
        ...
        if (!price || !maxParticipants || !time)
            throw `Por favor, introduzca todos los parámetros correctamente!`;
    ...
    if (command === 'enter') {
        ...
        if (!giveaway || giveaway.giveawayId !== giveawayId)
            throw `No hay sorteos aún!`;
        if (giveaway.isEnded)
            throw `El sorteo ya acabó!`;
...
let endGiveaway = (groupId, conn, reason) => {
    ...
    if (giveaway.participants.length > 0) {
        ...
        saveGiveawayData();
        conn.reply(groupId, `🎉 SORTEO TERMINADO 🎉\n\n` +
            `! Ganador: @${winner.split('@')[0]}\n` +
            `! Premio: ${giveaway.price}\n` +
            `! Anfitrión: @${giveaway.host.split('@')[0]}\n\n` +
            `! Nota: ¡Felicitaciones al ganador! Ponte en contacto con el anfitrión para reclamar la recompensa.`, null, { mentions: [winner, giveaway.host] });
    }
    else if (reason === 'timeout') {
        conn.reply(groupId, `⏰ Tiempo acabado, SORTEO TERMINADO ⏰\n\n` +
            `! Premio: ${giveaway.price}\n` +
            `! Anfitrión: @${giveaway.host.split('@')[0]}\n\n` +
