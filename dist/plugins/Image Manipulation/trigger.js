    ...
    start: async (Yaka, m, { text, prefix, quoted, pushName, mime, body }) => {
        ...
        const getImage = async (image, timeout = 15) => {
            ...
            for (let i = 0; i < 9; i++) {
                ...
                ctx.drawImage(await Canvas.loadImage(triggerd || Buffer.from('')), Math.floor(Math.random() * LR) - LR, 310 - 54 + Math.floor(Math.random() * LR) - LR, 256 + LR, 54 + LR);
                GIF.addFrame(ctx);
            }
            GIF.finish();
        ...
        try {
            ...
            if (!stickerm)
                return m.reply(`I couldn't find an image to trigger.`);
