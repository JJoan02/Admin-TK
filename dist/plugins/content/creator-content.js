...
export const textProMessages = {
    usage: (command, example1, example2, effectsList) => `USAR EL COMANDO DE SIGUIENTE MANERA\nUSE THE COMMAND AS FOLLOWS\n💫 *${command} efecto texto*\nEJEMPLO | EXAMPLE\n💫 *${command} ${example1}*\n\nEN CASO DE QUE DIGA QUE FALTA TEXTO USAR DE ESTA FORMA\nIN CASE IT SAYS TEXT IS MISSING USE LIKE THIS\n💫 *${command} efecto texto1|texto2*\nEJEMPLO | EXAMPLE\n💫 *${command} ${example2}*\n\n*✨ LISTA DE LOGOS*\n\n🤍 ${effectsList}`,
    effectNotFound: (effectName) => `${global.mg}EL LOGO ${effectName} NO SE ENCUENTRA EN LA LISTA DE LOGOS\n\nTHE LOGO ${effectName} IT IS NOT IN THE LIST OF LOGOS`,
    missingText: `${global.mg}FALTA EL TEXTO AL QUE SE REALIZARA EL LOGO`,
    creatingLogo: `${global.eg}ESPERE UN MOMENTO POR FAVOR SU LOGO SE ESTA CREANDO\n\nWAIT A MOMENT, YOUR VIDEO IS DOWNLOADING`,
    success: (effectName) => `✅ *AQUÍ ESTA TÚ LOGO!!*\n✅ *HERE IS YOUR LOGO!!*\n\n🔆 *EFECTO: ${effectName}*\n${global.wm}`,
    error: (command) => `${global.lenguajeTK.smsMalError3()}\n*${global.lenguajeTK.smsMensError1()}*\n*${global.lenguajeTK.smsMensError2()} ${command}`
};
export const pixelateImageMessages = {
    success: '🌀 *AQUÍ ESTA LA IMAGEN PIXELADA!!*\n𝙂𝙖𝙩𝙖𝘽𝙤𝙩-𝙈𝘿 | 𝙂𝙖𝙩𝙖 𝘿𝙞𝙤𝙨',
};
export const textToImageMessages = {
    noText: (usedPrefix, command) => `${global.lenguajeTK.smsAvisoMG()} 𝙌𝙐𝙀 𝙀𝙎𝘾𝙍𝙄𝘽𝙄𝙊? 𝙐𝙎𝘼𝙍 𝙀𝙎𝙏𝙀 𝘾𝙊𝙈𝘼𝙉𝘿𝙊 𝘿𝙀 𝙇𝘼 𝙎𝙄𝙂𝙐𝙄𝙀𝙉𝙏𝙀 𝙁𝙊𝙍𝙈𝘼\n\n𝙀𝙅𝙀𝙈𝙋𝙇𝙊: *${usedPrefix + command}* Hola GataBot`,
    success: (wm) => `✍🏻 𝙀𝙎𝙏𝘼 𝙇𝙄𝙎𝙏𝙊!!\n${wm}`,
};
export const ephoto360Messages = {
    noText: (mg) => `${mg}𝙀𝙎𝘾𝙍𝙄𝘽𝘼 𝙐𝙉 𝙏𝙀𝙓𝙏𝙊 𝙋𝘼𝙍𝘼 𝘾𝙍𝙀𝘼𝙍 𝙀𝙇 𝙇𝙊𝙂𝙊\n\n𝙒𝙍𝙄𝙏𝙀 𝘼 𝙏𝙀𝙓𝙏 𝙏𝙊 𝘾𝙍𝙀𝘼𝙏𝙀 𝙏𝙃𝙀 𝙇𝙊𝙂𝙊`,
    creating: (eg) => `${eg}𝙀𝙎𝙋𝙀𝙍𝙀 𝙐𝙉 𝙈𝙊𝙈𝙀𝙉𝙏𝙊 𝙋𝙊𝙍 𝙁𝘼𝙑𝙊𝙍 𝙎𝙐 𝙇𝙊𝙂𝙊 𝙎𝙀 𝙀𝙎𝙏𝘼 𝘾𝙍𝙀𝘼𝙉𝘿𝙊\n\n𝙒𝘼𝙄𝙏 𝘼 𝙈𝙊𝙈𝙀𝙉𝙏, 𝙔𝙊𝙐𝙍 𝙑𝙄𝘿𝙀𝙊 𝙄𝙎 𝘿𝙊𝙒𝙉𝙇𝙊𝘼𝘿𝙄𝙉𝙂`,
    error: (fg) => `${fg}𝘼𝙇𝙂𝙊 𝙎𝘼𝙇𝙄𝙊 𝙈𝘼𝙇 𝙑𝙐𝙀𝙇𝘼𝙑𝘼 𝙄𝙉𝙏𝙀𝙉𝙏𝘼𝙍\n\n𝙎𝙊𝙈𝙀𝙏𝙃𝙄𝙉𝙂 𝙒𝙀𝙉𝙏 𝙒𝙍𝙊𝙉𝙂 𝙏𝙍𝙔 𝘼𝙂𝘼𝙄𝙉`,
};
//# sourceMappingURL=creator-content.js.map