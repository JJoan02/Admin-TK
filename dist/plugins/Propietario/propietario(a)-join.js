...
let handler = async (m, { conn, text, isOwner }) => {
    ...
    if (botConfig.prestar === false && isOwner) {
        ...
        time = timeMatch ? parseInt(timeMatch[1]) : 60;
    ...
    if (botConfig.prestar === true || isOwner) {
        if (!isOwner) {
            ...
            if (user.limit < cost)
                return m.reply(`❌ NO TIENE SUFICIENTE DIAMANTE, NECESITA *${cost} DIAMANTES* PARA UNIRTE AL GRUPO.`);
            user.limit -= cost;
...
//# sourceMappingURL=propietario(a)-join.js.map