let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0] || isNaN(args[0]))
        return m.reply(`🚩 Ingresa un número que represente el número de días.\n\n*Ejemplo:*\n*${usedPrefix + command}* 30`);
    let who;
    if (m.isGroup)
    ...
    if (now < global.db.data.chats[who].expired)
        global.db.data.chats[who].expired += nDays;
    else
        global.db.data.chats[who].expired = now + nDays;
...
//# sourceMappingURL=owner-addexpired.js.map