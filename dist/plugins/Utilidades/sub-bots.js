...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
    if (m.fromMe || conn.user.jid === m.sender)
        return;
            ...
            if (connection === 'close') {
                ...
                if (reason === 440) {
                    ...
                    if (options.fromCommand)
                        m?.chat ? await conn.sendMessage(m.chat, { text: '*⚠️ HEMOS DETECTADO UNA NUEVA SESIÓN, BORRE LA NUEVA SESIÓN PARA CONTINUAR*\n\n> *SI HAY ALGÚN PROBLEMA VUELVA A CONECTARSE*' }, { quoted: m || null }) : "";
                }
                if (reason == 405 || reason == 401) {
                    ...
                    if (options.fromCommand)
                        return m?.chat ? await conn.sendMessage(m.chat, { text: '*🟢 REENVIAR NUEVAMENTE EL COMANDO....*' }, { quoted: m || null }) : '';
                }
                if (reason === 500) {
...
async function checkSubBots() {
    ...
    for (const folder of subBotFolders) {
        ...
        if (!fs.existsSync(credsPath))
            continue;
        const isAlreadyConnected = global.conns.find(conn => conn.user?.jid?.includes(folder) || path.basename(pathGataJadiBot) === folder);
...
//# sourceMappingURL=sub-bots.js.map