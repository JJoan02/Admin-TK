...
let handler = async (m, { command, usedPrefix, text, conn }) => {
    ...
    if (fs.existsSync(fantasyDBPath)) {
        fantasyDB.forEach(user => {
            ...
            if (flow) {
                flow.forEach(voto => {
                    if (voto.character_name === nombre && voto.like)
                        cantidadLikes++;
                    if (voto.character_name === nombre && voto.superlike)
                        cantidadSuperlikes++;
                    if (voto.character_name === nombre && voto.dislike)
                        cantidadDislikes++;
                });
            }
        });
...
//# sourceMappingURL=rpg-fantasy_info.js.map