...
let handler = async (m, { command, usedPrefix, conn, text }) => {
    ...
    const sortedCharactersByClass = Object.fromEntries(Object.entries(charactersByClass).sort((a, b) => classOrder.indexOf(a[0]) - classOrder.indexOf(b[0])));
    ...
    let totalPages = 1;
    const maxSectionLength = Math.max(allCharacters.split('\n').length, ...Object.values(sortedCharactersByClass).map((characters) => characters.length), ...Object.values(charactersByType).map((characters) => characters.length), lowCostCharacters.length, highCostCharacters.length);
...
//# sourceMappingURL=rpg-fantasy_list.js.map