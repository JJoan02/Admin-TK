...
let handler = async (m, { conn, args, text, users, user, usedPrefix, command, isPrems, isOwner, isROwner }) => {
    try {
        ...
        if (!global.db.data.settings[conn.user.jid].temporal)
            return await conn.reply(m.chat, texto0, fkontak, m);
        if (!args[0])
            return await conn.reply(m.chat, texto1, fkontak, m);
        let [_, code] = args[0].match(linkRegex) || [];
        if (!linkRegex.test(args[0]))
            return await conn.reply(m.chat, texto3, fkontak, m);
        ...
        if (!(isPrems || isOwner || isROwner)) {
            ...
            if (isNaN(args[1]))
                return conn.sendButton(m.chat, wm, texto4, img1, [[`🍀 𝙈 𝙀 𝙉 𝙐`, `.menu`]], fkontak, m);
            if (args[1] < 3)
                return conn.sendButton(m.chat, wm, texto5, img2, [[`🍀 𝙈 𝙀 𝙉 𝙐`, `.menu`]], fkontak, m);
            if (args[1] > 3)
                return conn.sendButton(m.chat, wm, texto6, img2, [[`🍀 𝙈 𝙀 𝙉 𝙐`, `.menu`]], fkontak, m);
            await delay(3 * 3000);
...
//# sourceMappingURL=rpg-bot_temporal.js.map