...
let handler = async (m, { command, usedPrefix, conn, text, args }) => {
    ...
    fantasyUsuario.forEach(personaje => {
        const infoPersonaje = data.infoImg.find(img => img.name.toLowerCase() === personaje.name.toLowerCase());
        if (infoPersonaje) {
            if (!clases[infoPersonaje.class])
                clases[infoPersonaje.class] = 0;
    ...
    for (const usuario of preciosPersonajes) {
        ...
        if (topUsuariosCaros.length >= cantidadUsuariosRanking)
            break;
    }
    ...
    let usuariosTransferencias = fantasyDB
        ...
        .filter(usuario => usuario.totalTransferencias > 0)
...
//# sourceMappingURL=rpg-fantasy_top_and_my.js.map