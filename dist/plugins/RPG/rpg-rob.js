...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    if (!(who in global.db.data.users))
        throw `${lenguajeGB['smsAvisoAG']()}𝙀𝙇 𝙐𝙎𝙐𝘼𝙍𝙄𝙊 𝙉𝙊 𝙎𝙀 𝙀𝙉𝘾𝙐𝙀𝙉𝙏𝙍𝘼 𝙀𝙉 𝙈𝙄 𝘽𝘼𝙎𝙀 𝘿𝙀 𝘿𝘼𝙏𝙊𝙎.`;
    let users = global.db.data.users[who];
    let rob = Math.floor(Math.random() * ro);
    if (users.exp < rob)
        return m.reply(`😿 @${who.split `@`[0]} tiene menos de *${ro} XP* No robes a un pobre :v`, null, { mentions: [who] });
...
//# sourceMappingURL=rpg-rob.js.map