const handler = async (m, { conn, command, args }) => {
    ...
    if (command == 'dep' || command == 'depositar') {
        if (!args[0])
            return m.reply(`*💎 INGRESA LA CANTIDAD PARA AGREGAR A SUS CUENTAS  BANCARIA*`);
        if (args[0] == '--all') {
        ...
        ;
        if (!Number(args[0]))
            return m.reply(`[ ⚠️ ] *FALTO EN NÚMERO DE CANTIDAD DE DIAMANTE 💎*`);
        let count = parseInt(args[0]);
        if (!users.limit)
            return m.reply(`*😿 ¡NO TIENES DIAMANTES SUFICIENTES, VAS A QUEDAR EN LA RUINA!*`);
        if (users.limit < count)
            return m.reply(`*😾 ¡¿CÓMO QUE NO TIENES SUFICIENTE DINERO?! RECUERDA QUE PUEDES VER TU BALANCE CON EL COMANDO:* #bal`);
        users.limit -= count * 1;
        users.banco += count * 1;
    ...
    if (command == 'retirar' || command == 'toremove') {
        ...
        if (!args[0])
            return m.reply(`[ ⚠️ ] *INGRESA LA CANTIDAD A RETIRAR*`);
        if (args[0] == '--all') {
        ...
        if (!Number(args[0]))
            return m.reply(`*¡LA CANTIDAD DEBE SER UN NÚMERO VÁLIDO!*`);
        let count = parseInt(args[0]);
        if (!user.banco)
            return m.reply(`*👻 ¡HEY FANTASMA! NO TIENES ESE DINERO EN EL BANCO, ESTÁS VACÍO! 😿*`);
        if (user.banco < count)
...
//# sourceMappingURL=rpg-dep-banco.js.map