let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (global.db.data.users[m.sender].pasangan == "")
        return await conn.reply(m.chat, `*${usuario}* 𝙉𝙊 𝙏𝙄𝙀𝙉𝙀𝙎 𝙋𝘼𝙍𝙀𝙅𝘼\n\n𝙔𝙊𝙐 𝘼𝙍𝙀 𝙉𝙊𝙏 𝙄𝙉 𝘼 𝙍𝙀𝙇𝘼𝙏𝙄𝙊𝙉𝙎𝙃𝙄𝙋\n\n> *_Si quiere tener una pareja use el comando ${usedPrefix}pareja etiquetando a alguien._*\n\n> ${wm}`, fkontak, m);
    try {
        if (global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender)
            return await conn.reply(m.chat, `*${usuario}* 💗 𝙀𝙎𝙏𝘼𝙎 𝙀𝙉 𝙐𝙉𝘼 𝙍𝙀𝙇𝘼𝘾𝙄𝙊𝙉 𝘾𝙊𝙉 *${await conn.getName(persona)}* 💖`, fkontak, m, { contextInfo: { mentionedJid: [m.sender, who] } });
...
//# sourceMappingURL=rol-rpg_pareja_mipareja.js.map