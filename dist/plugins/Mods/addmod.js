...
module.exports = {
    ...
    start: async (Yaka, m, { text, prefix, mentionByTag, pushName, isCreator, owner, modStatus }) => {
        ...
        if (!text && !m.quoted) {
            return Yaka.sendMessage(m.from, { text: `Please tag a user to make *Mod*!` }, { quoted: m });
        }
        else if (m.quoted) {
            ...
            mku
                ...
                if (!user) {
                    ...
                    return Yaka.sendMessage(m.from, {
                        text: `@${mentionedUser.split("@")[0]} has been added to *Mods* Successfully !`,
                        mentions: [mentionedUser],
                    }, { quoted: m });
                }
                else if (user.addedMods === null ||
                ...
                .catch((error) => {
                console.log(error);
                return Yaka.sendMessage(m.from, { text: `An internal error occurred while adding the user.` }, { quoted: m });
            });
        }
        catch (err) {
            console.log(err);
