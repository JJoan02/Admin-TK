...
module.exports = {
    ...
    usage: "ban @user",
    react: "🍃",
    start: async (Yaka, m, { text, prefix, isBotAdmin, isAdmin, mentionByTag, metadata, pushName, isCreator, args, modStatus }) => {
        ...
        try {
            mku.findOne({ id: userId }).then(async (user) => {
                if (!user) {
                    if (modStatus == "true" || ownerlist.includes(`${mentionedUser.split("@")[0]}`))
                        return Yaka.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} is a *Mod* and can't be banned !`, mentions: [mentionedUser] }, { quoted: m });
                    await mku.create({ id: userId, ban: true, reason: banreason, gcname: GroupName });
                    return Yaka.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} has been *Banned* Successfully by *${pushName}*\n\n *Reason*: ${banreason}`, mentions: [mentionedUser] }, { quoted: m });
                }
                else {
                    if (modStatus == "true" || ownerlist.includes(`${mentionedUser.split("@")[0]}`))
                        return Yaka.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} is a *Mod* and can't be banned !`, mentions: [mentionedUser] }, { quoted: m });
                    if (user.ban == "true")
                        return Yaka.sendMessage(m.from, { text: `@${mentionedUser.split("@")[0]} is already *Banned* !`, mentions: [mentionedUser] }, { quoted: m });
