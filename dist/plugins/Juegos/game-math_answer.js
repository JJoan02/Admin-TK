...
const handler = async (m, { conn }) => {
    ...
    if (m.quoted.sender != conn.user.jid)
        return;
    if (!/^Cuanto es el resultado de/i.test(m.quoted.text))
        return;
    if (!(m.chat in global.math))
        return conn.reply(m.chat, `🌵 Ya se ha respondido a esa pregunta.`, m, rcanal);
...
//# sourceMappingURL=game-math_answer.js.map