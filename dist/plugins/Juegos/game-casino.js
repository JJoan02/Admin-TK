...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
    ...
    if (args.length < 1)
        return conn.reply(m.chat, '🚩 Ingresa la cantidad de ' + `🍪 *Cookies*` + ' que deseas aportar contra' + ` *YaemoriBot-MD*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal);
    if (user.cookies >= count * 1) {
        ...
        conn.reply(m.chat, `No tienes *${formatNumber(count)} 🍪 Cookies* para apostar!`.trim(), m, rcanal);
};
...
//# sourceMappingURL=game-casino.js.map