...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        ...
        for (let x of anu) {
            if (c == 0)
                await conn.sendMessage(m.chat, { image: { url: x }, caption: `✅ ${mid.smsTikTok5(anu)}` }, { quoted: m });
            else
                await conn.sendMessage(m.sender, { image: { url: x } }, { quoted: m });
            c += 1;
        }
    }
    catch (e) {
...
//# sourceMappingURL=descargas-tiktokimg.js.map