...
const handler = async (m, { conn, command }) => {
    ...
    try {
        if (isPropose) {
            ...
            if (userIsMarried(proposer))
                throw new Error(`Ya estás casado con ${conn.getName(marriages[proposer])}.`);
            if (userIsMarried(proposee))
                throw new Error(`${conn.getName(proposee)} ya está casado con ${conn.getName(marriages[proposee])}.`);
            ...
            if (!userIsMarried(m.sender))
                throw new Error('No estás casado con nadie.');
