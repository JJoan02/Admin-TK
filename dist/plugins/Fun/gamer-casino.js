...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
    ...
    if (args.length < 1)
        return conn.reply(m.chat, '🍭 Ingresa la cantidad de ' + `*🍬 Dulces*` + ' que deseas aportar contra' + ` *Sumi Sakurasawa*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal);
    if (user.limit >= count * 1) {
        ...
        conn.reply(m.chat, `No tienes *${formatNumber(count)} 🍬 dulces* para apostar!`.trim(), m, rcanal);
};
...
//# sourceMappingURL=gamer-casino.js.map