    ...
    start: async (Yaka, m, { args, isBotAdmin, isAdmin, isCreator, reply, prefix, pushName }) => {
        ...
        if (args[0] === "on") {
            if (!checkdata) {
                ...
                return Yaka.sendMessage(m.from, { text: `*Successfully activated antilink*` }, { quoted: m });
            }
            else {
                if (checkdata.antilink == "true")
                    return Yaka.sendMessage(m.from, { text: `*Already activated.*` }, { quoted: m });
                await mk.updateOne({ id: m.from }, { antilink: "true" });
                return Yaka.sendMessage(m.from, { text: `*Antilink is enabled in this group*` }, { quoted: m });
            }
        }
        else if (args[0] === "off") {
            if (!checkdata) {
                await new mk({ id: m.from, antilink: "false" }).save();
                return Yaka.sendMessage(m.from, { text: `*Successfully deactivated antilink*` }, { quoted: m });
            }
            else {
                ...
                return Yaka.sendMessage(m.from, { text: `*Antilink is disabled in this group*` }, { quoted: m });
            }
        }
        else {
            let buttonsntilink = [
...
//# sourceMappingURL=antilinkgc.js.map