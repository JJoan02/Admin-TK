...
module.exports = {
    ...
    start: async (Yaka, m, { text, prefix, mentionByTag, args, pushname, owner, includes }) => {
        ...
        switch (action) {
            case "create":
                if (!playlistName)
                    return Yaka.sendMessage(m.from, { text: `Please provide a name for the playlist` }, { quoted: m });
            ...
            case "add":
                if (!playlistName)
                    return Yaka.sendMessage(m.from, { text: `Please provide the name of the playlist to add the song to to` }, { quoted: m });
                if (!songUrl)
                    return Yaka.sendMessage(m.from, { text: `Please provide the url of the song to add` }, { quoted: m });
                ...
                playlist.findOne({ name: playlistName, owner: m.sender })
                    .then(async (pl) => {
                    if (!pl)
                        return Yaka.sendMessage(m.from, { text: `Playlist not found` }, { quoted: m });
                    pl.songs.push({
                        title: song.title,
                        url: song.url,
                        savedAt: new Date()
                    ...
                    if (!song.title)
                        return Yaka.sendMessage(m.from, { text: `title not found` }, { quoted: m });
                    await pl.save();
            ...
            case "remove":
                if (!playlistName)
                    return Yaka.sendMessage(m.from, { text: `Please provide the name of the playlist to remove the song from` }, { quoted: m });
                if (!songUrl)
                    return Yaka.sendMessage(m.from, { text: `Please provide the url of the song to remove` }, { quoted: m });
                playlist.findOne({ name: playlistName, owner: m.sender })
                    .then(async (pl) => {
                    if (!pl)
                        return Yaka.sendMessage(m.from, { text: `Playlist not found` }, { quoted: m });
            ...
            case "view":
                if (!playlistName)
                    return Yaka.sendMessage(m.from, { text: 'Please provide the name of the playlist to view' }, { quoted: m });
                playlist.findOne({ name: playlistName, owner: m.sender })
                    .then((pl) => {
                    if (!pl)
                        return Yaka.sendMessage(m.from, { text: 'Playlist not found' }, { quoted: m });
