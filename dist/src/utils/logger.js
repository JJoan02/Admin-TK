...
export const setConsoleSilentMode = (silent) => {
    silentMode = silent;
    if (silent) {
...
export const logSystemEvent = (event, data, level = 'info') => {
    logger[level]({
        event,
        data,
...
export const logCommand = (command, user, group, success = true) => {
    logger.info({
        command,
        user,
        group,
...
export const logPluginError = (pluginName, error, context) => {
    logger.error({
        plugin: pluginName,
        error: {
...
export const logAPIRequest = (method, path, user, responseTime, statusCode) => {
    logger.info({
        method,
        path,
        user,
