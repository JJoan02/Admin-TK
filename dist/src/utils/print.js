// src/utils/print.js
import { WAMessageStubType, jidNormalizedUser, getContentType } from '@whiskeysockets/baileys';
        ...
        case 'chats.update': {
            for (const update of data) {
                ...
                if (update.unreadCount !== undefined)
                    action = `Mensajes no leídos: ${update.unreadCount}`;
                if (update.mute !== undefined)
                    action = `Chat ${update.mute ? 'silenciado' : 'desilenciado'}`;
                if (update.archive !== undefined)
                    action = `Chat ${update.archive ? 'archivado' : 'desarchivado'}`;
                if (update.pin !== undefined)
                    action = `Chat ${update.pin ? 'fijado' : 'desfijado'}`;
