...
export class KickCommandHandler {
    async handle(command) {
        ...
        try {
            ...
            let responseText = '';
            if (success.length > 0)
                responseText += `✅ Se expulsó a: ${success.join(', ')}
`;
            if (failed.length > 0)
                responseText += `❌ Falló al expulsar a: ${failed.join(', ')}
`;
...
//# sourceMappingURL=KickCommandHandler.js.map