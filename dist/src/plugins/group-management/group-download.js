// group-download.ts - Plugin mejorado y optimizado
...
module.exports = async (Yaka, m, commands, chatUpdate, store) => {
    try {
        ...
        let body = type == "buttonsResponseMessage"
            ...
            : type == "listResponseMessage"
                ...
                : type == "templateButtonReplyMessage"
                    ...
                    : m.text;
        let prat = type === "conversation" && body?.startsWith(prefix)
            ? body
        ...
        const cmd = commands.get(cmdName) ||
            ...
            "";
        const icmd = commands.get(cmdName) ||
            Array.from(commands.values()).find((v) => v.alias.find((x) => x.toLowerCase() == cmdName));
        const mentionByTag = type == "extendedTextMessage" &&
            ...
            : [];
        if (!isCreator) {
            let checkban = (await mku.findOne({
                id: m.sender,
            })) ||
                (await new mku({
                    id: m.sender,
            ...
            if (isCmd &&
                ...
                budy != `${prefix}mod` &&
                budy != `${prefix}modlist`)
                return m.reply(`You are *Banned* from using commands for *${checkban.reason}* from *${checkban.gcname}*`);
        }
        ...
        if (checkdata) {
            ...
            if (m.isGroup && mongoschema == "true") {
                ...
                if (budy.includes(`https://chat.whatsapp.com/${linkgce}`)) {
                    m.reply(`No action will be taken because you sent this group's link.`);
                }
                    ...
                    Yaka.sendMessage(from, {
                        text: `\`\`\`*Antilink System*\`\`\`\n\n@${kice.split("@")[0]} Removed for sending WhatsApp group link in this group! Message deleted.`,
                        mentions: [kice],
                    }, {
                        quoted: m,
                    });
            ...
            if (m.isGroup && csts == "true" && !icmd && !isCmd) {
                if (m.quoted) {
                    if (m.quoted.sender == botNumber) {
                        const botreply = await axios.get(`http://api.brainshop.ai/get?bid=174300&key=ugPE0tD90fafvu2N&uid=[uid]&msg=[${budy}]`);
            ...
            if (timestamps.has(m.sender)) {
                ...
                if (now < expiration) {
                    ...
                    return await Yaka.sendMessage(m.from, {
                        text: `Don't Spam ! You can use command after _${timeLeft.toFixed(1)} second(s)_`,
                    }, {
                        quoted: m,
                    });
                }
...
export default {};
//# sourceMappingURL=group-download.js.map