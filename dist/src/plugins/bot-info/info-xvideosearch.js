    ...
    async execute(context) {
        ...
        try {
            if (!db.data.chats[m.chat].nsfw && m.isGroup)
                return await reply(m.chat, '🚩 *¡Estos comandos están desactivados!*', m, fake);
            if (!text)
                return await reply(m.chat, 'Ingresa el texto de lo que quieres buscar en Xvideo 🤍', m, rcanal);
            await m.react('🕓');
            try {
