...
export class jadibotsunoPlugin extends BasePlugin {
    ...
    async execute(context) {
        ...
        try {
            if (!text)
                return await reply(`Ejemplo:.suno canción sobre lo que siento por ella, cantante masculino, estilo lofi relajado`);
            await reply('🔄 Generando...');
            try {
                let result = await suno(text);
                if (!result?.data?.length)
                    return await reply('❌ No se pudo obtener la canción');
...
//# sourceMappingURL=jadibot-suno.js.map