...
export class utilcapcutPlugin extends BasePlugin {
    ...
    async execute(context) {
        ...
        try {
            if (!args[0])
                return await reply(m.chat, `[ ✰ ]  Ingresa un link de CapCut`, m);
            if (!args[0].match(/capcut/gi))
                return await reply(m.chat, `[ ✰ ]  Verifica que el link sea de *CapCut*`, m);
            await m.react('🕓');
            try {
                const response = await axios.get(`https://api.siputzx.my.id/api/d/capcut?url=${encodeURIComponent(args[0])} catch (error) {
...
//# sourceMappingURL=util-capcut.js.map