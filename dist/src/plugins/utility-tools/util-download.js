// util-download.ts - Plugin mejorado y optimizado
...
const bestFormat = (url, type) => new Promise(async (resolve, reject) => {
    ...
    if (type == 'audio') {
        ...
        if (!format)
            format = formatau.find((format) => format.quality == 128);
        if (!format)
            format = formatau.find((format) => format.quality == 64);
        if (!format)
            format = formatau.find((format) => format.quality == 48);
        if (!format)
            return reject(false);
...
export default {};
//# sourceMappingURL=util-download.js.map