...
export class admindepositPlugin extends BasePlugin {
    ...
    async execute(context) {
        ...
        try {
            ...
            if (!args[0])
                return await reply('🚩 Ingresa la cantidad de *Chocolates 🍫* que deseas Depositar.');
            if ((args[0]) < 1)
                return await reply('🚩 Ingresa una cantidad válida de *Chocolates 🍫*.');
            if (args[0] == 'all') {
                let count = parseInt(user.chocolates);
                user.chocolates -= count * 1;
                user.bank += count * 1;
                await await reply(`Depositaste *${count} catch (error) {
...
//# sourceMappingURL=admin-deposit.js.map