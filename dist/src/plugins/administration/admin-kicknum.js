...
const handler = async (m, { conn, args, groupMetadata, participants, usedPrefix, command, isBotAdmin, isSuperAdmin }) => {
    ...
    if (ps == '')
        return m.reply(`🚩 *Aqui No Hay Ningun Numero Con El Prefijo +${lol}*`);
    ...
    switch (command) {
        ...
        case 'kicknum':
            if (!bot.restrict)
                return conn.reply(m.chat, '🚩 *¡Este Comando Esta Desabilitado Por El Propietario Del Bot!*', m, rcanal);
            if (!isBotAdmin)
            ...
            for (const user of users) {
                ...
                if (user !== ownerGroup + '@s.whatsapp.net' && user !== global.conn.user.jid && user !== global.owner + '@s.whatsapp.net' && user.startsWith(lol || lol) && user !== isSuperAdmin && isBotAdmin && bot.restrict) {
                    ...
                    return m.reply(m.chat, '✖️ *Ocurrió un error.*', m, fake);
            }
            break;
    }
};
...
//# sourceMappingURL=admin-kicknum.js.map