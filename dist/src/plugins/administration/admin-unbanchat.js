...
export class adminunbanchatPlugin extends BasePlugin {
    ...
    async execute(context) {
        ...
        try {
            if (!(m.chat in await this.database.getData().chats))
                return await reply(m.chat, '🍭 *¡ESTE CHAT NO ESTÁ REGISTRADO!*', m, fake);
            let chat = await this.database.getData().chats[m.chat];
            if (!chat.isBanned)
                return await reply(m.chat, '🍟 *¡MEGUMIN-BOT NO ESTÁ BANEADA EN ESTE CHAT!*', m, fake);
            chat.isBanned = false;
...
//# sourceMappingURL=admin-unbanchat.js.map