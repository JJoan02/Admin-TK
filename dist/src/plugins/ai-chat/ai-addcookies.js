    ...
    async execute(context) {
        ...
        try {
            ...
            if (m.isGroup)
                ...
                who = m.chat;
            if (!who)
                return await reply('⚠️️ *Taguea al usuario*');
            let txt = text.replace('@' + who.split `@`[0], '').trim();
            if (!txt)
                return await reply('⚠️️ Ingrese la cantidad de *Galletas* que quiere añadir');
            if (isNaN(txt))
                return await reply('⚠️ *sólo números*');
            let dmt = parseInt(txt);
            let cookies = dmt;
            let pjk = Math.ceil(dmt * impts);
            cookies += pjk;
            if (cookies < 1)
                return await reply('⚠️️ Mínimo es  *1*');
            let users = await this.database.getData().users;
            users[who].cookies += dmt;
            await await reply(m.chat, `⊜ *🍪 AÑADIDO*
      ┏━━━━━━━━━━━⬣
      ┃⋄ *Total:* ${dmt} catch (error) {
...
//# sourceMappingURL=ai-addcookies.js.map