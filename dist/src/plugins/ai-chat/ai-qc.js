// ai-qc.ts - Plugin mejorado y optimizado
// Categoría: ai-chat
...
const handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    if (args.length >= 1) {
        ...
        return conn.reply(m.chat, '🚩 Te Faltó El Texto!', m, rcanal);
    if (!text)
        return conn.reply(m.chat, '🚩 Te Faltó El Texto!', m, rcanal);
    ...
    if (mishi.length > 40)
        return conn.reply(m.chat, '🚩 El texto no puede tener mas de 30 caracteres', m, rcanal);
    ...
    if (stiker)
        return conn.sendFile(m.chat, stiker, 'error.webp', '', fkontak);
};
handler.help = ['qc *<texto>*'];
handler.tags = ['sticker'];
handler.command = ['qc'];
export default handler;
//# sourceMappingURL=ai-qc.js.map