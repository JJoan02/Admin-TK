// dl-yts.ts - Plugin mejorado y optimizado
// Categoría: downloads
...
const handler = async (m, { conn, text }) => {
    if (!text.trim())
        return conn.reply(m.chat, "💣 Ingresa el nombre de la música a buscar.", m);
...
handler.before = async (m, { conn }) => {
    if (!m.quoted || !m.quoted.text.includes("🎵 *Resultados de búsqueda:*"))
        return;
