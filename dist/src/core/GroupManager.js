    ...
    async getGroup(jid, metadata = {}) {
        ...
        try {
            ...
            if (!group) {
                ...
                await this.#db.run('INSERT OR IGNORE INTO groups (jid, subject, creation, owner, size, isMuted, welcomeMessage, goodbyeMessage, isBotEnabled, isAiEnabled, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', newGroup.jid, newGroup.subject, newGroup.creation, newGroup.owner, newGroup.size, newGroup.isMuted, newGroup.welcomeMessage, newGroup.goodbyeMessage, newGroup.isBotEnabled, newGroup.isAiEnabled, newGroup.createdAt, newGroup.updatedAt);
