...
export class AIService {
    ...
    constructor(memoryService, config, logger, errorHandler) {
        ...
        // Usar la API key desde variables de entorno o config
        const apiKey = process.env.GEMINI_API_KEY || this.#config.geminiApiKey || this.#config.api?.geminiApiKey;
        if (!apiKey) {
    ...
    async _generateStandardResponse(context) {
        ...
        const historyPrompt = recentInteractions.reverse().map(interaction => `${interaction.is_from_bot ? 'Admin-TK' : interaction.user_name || interaction.user_jid.split('@')[0]}: ${interaction.message_text}`).join('\n');
