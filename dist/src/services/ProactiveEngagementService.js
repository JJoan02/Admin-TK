...
export class ProactiveEngagementService {
    ...
    async checkAndSend(sock) {
        ...
        try {
            const chats = await db.all("SELECT * FROM chats WHERE isAiEnabled = 1 AND jid NOT LIKE '%@g.us%'");
            ...
            for (const chat of chats) {
                ...
                if (hoursDiff >= requiredHours) {
                    ...
                    this.#logger.info(`💬 Enviando mensaje proactivo nivel ${currentLevel + 1} a ${chat.jid}. Han pasado ${hoursDiff.toFixed(2)} horas.`);
                    try {
...
//# sourceMappingURL=ProactiveEngagementService.js.map